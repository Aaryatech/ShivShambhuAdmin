...
<body onload="showOrderItemPopup1(${editChalan.orderId})">
								...
								<div class="row">
									...
									<div class="col-md-4">
										<select id="plant_id" name="plant_id" class="standardSelect"
											...
											<c:forEach items="${plantList}" var="plant">
												<c:choose>
													<c:when test="${editChalan.plantId==plant.plantId}">
									...
									<div class="col-md-4">
										<select id="cust_name" name="cust_name" class="standardSelect"
											...
											onchange="getCustInfo(1)">


											<c:forEach items="${custList}" var="cust">
												<c:choose>
													<c:when test="${editChalan.custId==cust.custId}">
								...
								<div class="row">
									...
									<div class="col-md-10">
										<select id="order_id" name="order_id" class="standardSelect"
											...
											<c:forEach items="${ordHeadList}" var="ordHead">

												<c:choose>
													<c:when test="${editChalan.orderId==ordHead.orderId}">
														<option selected value="${ordHead.orderId}">${ordHead.orderNo}--${ordHead.orderDate}</option>
													</c:when>
													<%-- <c:otherwise>
														<option value="${ordHead.orderId}">${ordHead.orderNo}</option>
													</c:otherwise> --%>
												</c:choose>
								...
								<div class="row">
									...
									<div class="col-md-4">
										<input type="text" autocomplete="off" id="chalan_date"
											readonly name="chalan_date" required style="width: 100%;"
									...
									<div class="col-md-4">
										<select id="veh_id" name="veh_id" style="width: 100%;"
											...
											<c:forEach items="${vehicleList}" var="veh">
												...
												<c:choose>
													<c:when test="${editChalan.vehicleId==veh.vehicleId}">
														<option selected value="${veh.vehicleId}">${veh.vehNo}-${veh.vehicleName}</option>
	...
	<script type="text/javascript">
	 ...
	 function setChalanQty(itemId,chDetailId,inputQty,remOrdQty,ordQty,itemQty){
		 alert("Inside setChalanQty")
		  if(parseFloat(inputQty)>remOrdQty){
			 alert("Chalan Quantity can not be greater than remaining order quantity ");
			 document.getElementById("chQty"+chDetailId).value=itemQty;
		 }
	 }

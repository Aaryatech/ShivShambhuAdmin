									...
									<table id="bootstrap-data-table"
										...
										<thead>
											<tr>
												...
												<th style="text-align: center">Production Batch</th>
											</tr>
										</thead>
										<tbody>
									<c:forEach items="${prodReportDetList}" var="prodDetailReport" varStatus="count">
										<tr>
											<td style="text-align: center">${count.index+1}</td>
											<td style="text-align: left"><c:out
													value="${prodDetailReport.itemName}" /></td>
											<td style="text-align: center"><c:out
													value="${prodDetailReport.uomName}" /></td>
													
													
													<td style="text-align: left"><c:out
													value="${prodDetailReport.productionDate}" /></td>
											<td style="text-align: center"><c:out
													value="${prodDetailReport.planQty}" /></td>
													
														<c:if test="${prodDetailReport.status==1}">
									<c:set var="status" value="Planned"></c:set>
								</c:if>
								
								<c:if test="${prodDetailReport.status==2}">
									<c:set var="status" value="Started"></c:set>
								</c:if>
								
								<c:if test="${prodDetailReport.status==3}">
									<c:set var="status" value="Completed"></c:set>
								</c:if>
									
													<td style="text-align: left"><c:out
													value="${prodDetailReport.productionQty}" /></td>
											<td style="text-align: center"><c:out
													value="${status}" /></td>
													
													<td style="text-align: center"><c:out
													value="${prodDetailReport.productionBatch}" /></td>
